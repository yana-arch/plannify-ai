      ...
      <main>
        {projects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((project) => (
              <Card key={project.id} className="flex flex-col h-full">
                <h3 className="text-lg font-semibold text-brand-primary-hover mb-2">{project.projectName}</h3>
                <p className="text-sm text-brand-text-secondary flex-grow mb-4 line-clamp-3">{project.shortDescription}</p>
          ...
          <div className="text-center py-16 border-2 border-dashed border-brand-border rounded-lg">
            ...
            <p className="text-brand-text-secondary mt-2 mb-6">You haven't generated any project plans yet.</p>
            <Button onClick={onNewProject}>
              <WandSparklesIcon className="h-4 w-4 mr-2" />
