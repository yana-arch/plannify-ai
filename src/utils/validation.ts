...
export const validators = {
  ...
  arrayRequired: (value: any[], fieldName: string): string | null => {
  ...
  timelineFormat: (value: string, fieldName: string): string | null => {
    if (value && !/\d+-\d+\s*months?|\d+-\d+\s*weeks?|\d+\s*months?|\d+\s*weeks?/.test(value.toLowerCase())) {
...
export const validateStepData = (step: number, data: Partial<ProjectInputData>): ValidationResult => {
  ...
  switch (step) {
    case 0: // Basic Information
      ...
      const descWarning = validators.wordCount(data.shortDescription || '', 10, 100, 'Project Description');
      ...
      const numErr = validators.numberRange(data.numberOfFeatures || 0, 3, 20, 'Number of Features');
      ...
      if (!data.coreRequirements || data.coreRequirements.length === 0) {
        ...
        data.coreRequirements.forEach((req, index) => {
          if (!req.description.trim()) {
            addError(`coreRequirements[${index}].description`, `Requirement ${index + 1} description cannot be empty`);
          } else if (req.description.length < 10) {
            addWarning(`coreRequirements[${index}].description`, `Requirement ${index + 1} description is quite brief`);
          }
        });

        if (data.coreRequirements.length > (data.numberOfFeatures || 10) + 3) {
          addWarning('coreRequirements', `You have ${data.coreRequirements.length} requirements but planned for ${(data.numberOfFeatures || 10)} features. Consider consolidating.`);
        }
      }
      break;
    ...
    case 6: // Risk Assessment & Metrics
      ...
      if (!data.riskAssessment || data.riskAssessment.length === 0) {
        addWarning('riskAssessment', 'Consider adding at least one risk assessment for better project planning');
      } else {
        data.riskAssessment.forEach((risk, index) => {
          if (!risk.risk.trim()) {
            addError(`riskAssessment[${index}].risk`, `Risk ${index + 1} description cannot be empty`);
          }
          if (!risk.mitigation.trim()) {
            addError(`riskAssessment[${index}].mitigation`, `Risk ${index + 1} must have a mitigation strategy`);
          }
        });
      }
      ...
      if (!data.successMetrics || data.successMetrics.length === 0) {
        addWarning('successMetrics', 'Consider defining success metrics to measure project outcomes');
      } else {
        data.successMetrics.forEach((metric, index) => {
          if (!metric.metric.trim()) {
            addError(`successMetrics[${index}].metric`, `Success metric ${index + 1} name cannot be empty`);
          }
          if (!metric.target.trim()) {
            addError(`successMetrics[${index}].target`, `Success metric ${index + 1} must have a target`);
          }
          if (!metric.timeframe.trim()) {
            addWarning(`successMetrics[${index}].timeframe`, `Success metric ${index + 1} should have a timeframe`);
          }
        });
      }
    ...
    case 7: // Market Analysis
      if (!data.marketAnalysis || data.marketAnalysis.trim().length < 50) {
        addWarning('marketAnalysis', 'Market analysis should be detailed (at least 50 characters) for better planning');
      }
...
const checkTechConflicts = (techStack: ProjectInputData['techStack']): string[] => {
  ...
  if (database.includes('SQLite') && techStack.otherTools.some(tool =>
...
export const getFieldTooltip = (fieldName: string): string => {
  const tooltips: Record<string, string> = {
    ...
    businessGoals: 'What business objectives will this project achieve? (revenue growth, customer satisfaction, etc.)',
    technicalGoals: 'What technical objectives should this project meet? (performance, scalability, security, etc.)',
...
export const validateCompleteForm = (data: ProjectInputData): ValidationResult => {
  ...
  if (data.coreRequirements.length > data.numberOfFeatures * 1.5) {
    allWarnings.push({
      ...
      message: 'You have many requirements relative to planned features. Consider consolidating some.',
      type: 'warning'
    });
  }
