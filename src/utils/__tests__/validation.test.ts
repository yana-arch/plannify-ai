...
describe("Validation Utilities", () => {
  describe("validators", () => {
    ...
    it("minLength returns null for sufficient length", () => {
      expect(
        validators.minLength("long enough string", 10, "Field"),
      ).toBeNull();
    });

    it("arrayRequired returns error for empty arrays", () => {
      expect(validators.arrayRequired([], "Field")).toBe(
  ...
  describe("validateStepData", () => {
    const mockData: Partial<ProjectInputData> = {
      ...
      shortDescription:
        "This is a sufficiently long description for the project.",
    ...
    it("returns errors for missing required fields in Step 0", () => {
      ...
      expect(result.errors).toContainEqual(
        expect.objectContaining({ field: "projectName" }),
      );
    });
    ...
    it("validates Step 1 (Core Requirements) correctly", () => {
      const step1Data = {
        coreRequirements: [
          { id: "1", description: "Requirement 1", priority: "High" } as any,
        ],
      };
