...
import {
  enhanceFeatureSpecification,
  generateReport,
  regenerateProjectPlan,
  optimizeDevelopmentPlan,
  fixMermaidCode,
...
const OverviewTab: React.FC<{
  ...
  return (
    <div className="space-y-8">
      ...
      >
        The AI will regenerate the entire project plan based on your new prompt. This will save the
        current version to history and replace it. Are you sure you want to continue?
      </Modal>
      <Card>
        ...
        {isEvolving && (
          <div className="mt-4 pt-4 border-t border-brand-border/50 space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="secondary" onClick={handleEvolveToggle}>
                Cancel
              </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <div className="text-sm text-brand-text-secondary space-y-1">
            <p>
              <strong>Frontend:</strong> {plan.recommendedTechStack.frontend.join(', ')}
            </p>
            <p>
              <strong>Backend:</strong> {plan.recommendedTechStack.backend.join(', ')}
            </p>
            <p>
              <strong>Database:</strong> {plan.recommendedTechStack.database.join(', ')}
            </p>
            <p>
              <strong>Other:</strong> {plan.recommendedTechStack.other.join(', ')}
            </p>
          </div>
        </Card>
        <Card>
          ...
          <ul className="list-disc list-inside space-y-1 text-brand-text-secondary">
            {plan.potentialChallenges.map((challenge, i) => (
        ...
        <Card>
          ...
          <ul className="list-disc list-inside space-y-1 text-brand-text-secondary">
            {plan.potentialOpportunities.map((opp, i) => (
              <li key={i}>{opp}</li>
            ))}
          </ul>
        </Card>
...
const FeatureCard: React.FC<{
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const updatedFeature = await enhanceFeatureSpecification(
        feature,
        prompt,
        projectContext,
        activeProvider,
  ...
  return (
    <Card className="transition-all duration-300 ease-in-out">
      <Modal
        ...
        isConfirming={isGenerating}
      >
        The AI will enhance this feature based on your prompt. This will save the current plan to
        history and replace this feature's details. Are you sure you want to continue?
      </Modal>

      <div className="flex justify-between items-start gap-4">
        <div className="flex-grow">
          <h4 className="text-md font-semibold text-brand-primary-hover mb-2">{feature.name}</h4>
          <p className="text-sm text-brand-text-secondary mb-4 whitespace-pre-wrap">
            {feature.description}
          </p>
        </div>
        <Button
      ...
      {isEnhancing && (
        <div className="mt-4 pt-4 border-t border-brand-border/50 space-y-3">
          <label
            htmlFor={`enhance-prompt-${feature.name.replace(/\s+/g, '-')}`}
            className="block text-sm font-medium text-brand-text-secondary"
          >
            How should the AI enhance this feature?
          </label>
          <input
          ...
          {error && <p className="text-sm text-red-400 mt-1">{error}</p>}
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" onClick={handleEnhanceToggle}>
              Cancel
            </Button>
            <Button onClick={() => setShowConfirmModal(true)} isLoading={isGenerating}>
...
const DevelopmentTab: React.FC<{
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const newMilestones = await optimizeDevelopmentPlan(
        milestones,
        optimizePrompt,
        projectContext,
        activeProvider,
  ...
  const colors = ['#2F81F7', '#3FB950', '#A371F7', '#DB61A2', '#F7B955', '#58A6FF'];
  ...
  return (
    <div className="space-y-6">
      <Modal
        ...
        isConfirming={isGenerating}
      >
        The AI will generate a new development timeline based on your prompt. This will save the
        current version to history and replace the existing milestones. Are you sure you want to
        continue?
      </Modal>
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-brand-text-secondary mt-1">
            An AI-generated Gantt chart visualizing the project milestones over an estimated{' '}
            {totalWeeks - 1}-week timeline.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      {isOptimizing && (
        <Card>
          <div className="space-y-3">
            <label
              htmlFor="optimize-prompt"
              className="block text-sm font-medium text-brand-text-secondary"
            >
              How should the AI optimize this timeline?
            </label>
            <input
              id="optimize-prompt"
              type="text"
            ...
            {error && <p className="text-sm text-red-400 mt-1">{error}</p>}
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="secondary" onClick={handleOptimizeToggle}>
                Cancel
              </Button>
              <Button onClick={() => setShowConfirmModal(true)} isLoading={isGenerating}>
      ...
      {milestones.length > 0 ? (
        <div className="w-full overflow-x-auto bg-brand-bg p-4 rounded-lg border border-brand-border">
          <div className="relative" style={{ minWidth: `${totalWeeks * 60}px` }}>
            {/* Week Headers */}
            <div
              className="grid sticky top-0 bg-brand-bg z-10"
              style={{ gridTemplateColumns: `repeat(${totalWeeks - 1}, minmax(60px, 1fr))` }}
            >
              {Array.from({ length: totalWeeks - 1 }, (_, i) => (
                <div
                  key={i}
                  className="text-center text-xs font-semibold text-brand-text-secondary py-2 border-b border-r border-brand-border/30"
                >
                  Week {i + 1}
                </div>
              ))}
                ...
                return (
                  <div key={index}>
                    <div
                      ...
                      style={{
                        gridTemplateColumns: `repeat(${totalWeeks - 1}, minmax(60px, 1fr))`,
                      }}
                    >
                      <div
                    ...
                    <div
                      className="mt-3 ml-2 text-xs text-brand-text-secondary border-l-2 pl-3"
        ...
        <Card>
          <p className="text-brand-text-secondary">
            No development milestones available to display.
          </p>
        </Card>
      )}
    </div>
        ...
        {isEditing ? (
          ...
          <div>
            <div
              ...
              ref={mermaidRef}
              key={plan.systemArchitectureMermaid}
            >
              ...
              <div className="text-center text-brand-text-secondary p-4">
                {renderError ? (
                  <>
                    ...
                    <pre className="mt-2 text-xs text-left bg-brand-surface p-3 rounded-md overflow-x-auto">
                      {renderError}
                    </pre>
                  </>
                ) : (
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-brand-text-secondary mt-1">
            Entity-Relationship Diagram showing the database schema with tables, columns, and
            relationships.
          </p>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-brand-text-secondary mt-1">
            This diagram illustrates a key user journey or process within the application, providing
            insight into the user experience from a high level.
          </p>
        </div>
      </div>
...
const reportTemplates: ReportTemplate[] = [
  {
    ...
    description:
      'A detailed document for the engineering team, covering architecture, data models, and API design.',
  ...
  },
  {
    ...
    title: 'Product Brief',
    description:
      "A concise summary for stakeholders, outlining the product's purpose, features, and market goals.",
  ...
  },
  {
    ...
    description:
      'A high-level overview for leadership and investors, focusing on business value and strategic goals.',
    persona: 'For Leadership',
    icon: BriefcaseIcon,
  },
];

const ReportsTab: React.FC<{ plan: ProjectPlan; projectName: string }> = ({
  plan,
  projectName,
}) => {
  const { activeProvider } = useSettings();
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-brand-text-secondary">
        Select a template to generate a tailored report using AI. Choose between Markdown (.md) or
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportTemplates.map((template) => (
          <Card key={template.id} className="flex flex-col">
            ...
            <p className="text-sm text-brand-text-secondary flex-grow mb-6">
...
const HistoryTab: React.FC<{
  ...
  if (history.length === 0) {
    return (
      <div className="space-y-6">
        ...
        <Card>
          <p className="text-brand-text-secondary">
            No previous versions have been saved. As you evolve the plan using the features in the
            'Overview' or 'Features' tabs, older versions will appear here.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-brand-text-secondary">
        You can restore any previous version of your project plan. Restoring a version will save the
        current plan to history.
      </p>
      <div className="space-y-4">
        <Card className="flex justify-between items-center border-brand-primary/50">
          <div>
            ...
            <p className="text-xs text-brand-text-secondary">
              This is the version you are currently viewing.
            </p>
          </div>
          <span className="text-xs font-medium bg-brand-primary/20 text-brand-primary-hover px-3 py-1 rounded-full">
            Active
          </span>
        </Card>
